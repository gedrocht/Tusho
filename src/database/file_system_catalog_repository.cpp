...
void FileSystemCatalogRepository::initialize_schema()
{
  ...
  database_connection_.execute_sql_statement(
    "CREATE INDEX IF NOT EXISTS index_file_system_entries_by_entry_name "
    "ON file_system_entries(entry_name);");

  database_connection_.execute_sql_statement(
    "CREATE INDEX IF NOT EXISTS index_file_system_entries_by_entry_type "
...
void FileSystemCatalogRepository::clear_catalog()
{
...
std::int64_t FileSystemCatalogRepository::begin_scan_run(const std::string &crawl_root_directory_path_text)
{
  sqlite3_stmt *prepared_statement = database_connection_.prepare_statement(
    "INSERT INTO scan_runs (crawl_root_directory_path) VALUES (?);");
...
void FileSystemCatalogRepository::record_file_system_entry(const FileSystemEntryRecord &file_system_entry_record)
{
  ...
  bind_text(
    insert_or_update_file_system_entry_statement_,
    9,
...
void FileSystemCatalogRepository::finish_scan_run(
  const std::int64_t scan_run_identifier,
  const std::uint64_t accessible_entry_count,
  const std::uint64_t inaccessible_entry_count)
{
  sqlite3_stmt *prepared_statement = database_connection_.prepare_statement(
