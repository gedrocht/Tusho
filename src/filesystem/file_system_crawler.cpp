...
FileSystemCrawler::FileSystemCrawler(
  FileSystemCatalogRepository &file_system_catalog_repository,
...
ScanStatistics FileSystemCrawler::crawl(const RunConfiguration &run_configuration)
{
  ...
  try
  {
    ...
    file_system_catalog_repository_.record_file_system_entry(
      create_file_system_entry_record(
        scan_statistics.scan_run_identifier,
        normalized_crawl_root_directory_path,
  ...
  catch (const std::exception &exception)
  {
    ...
    structured_logger_.log_warning(
      "The crawler could not read the root directory metadata.",
      {
        {"crawl_root_directory_path", normalized_crawl_root_directory_path.string()},
  ...
  std::filesystem::recursive_directory_iterator current_entry_iterator(
    normalized_crawl_root_directory_path,
    iterator_options,
    iterator_construction_error_code);

  if (iterator_construction_error_code)
  {
    throw std::runtime_error(
      "Unable to begin crawling the requested root directory: " + iterator_construction_error_code.message());
  }
  ...
  while (current_entry_iterator != end_iterator)
  {
    ...
    else
    {
      ...
      catch (const std::exception &exception)
      {
        ++scan_statistics.inaccessible_entry_count;
        structured_logger_.log_warning(
          "The crawler discovered an entry but could not fully read it.",
          {
    ...
    if (iterator_increment_error_code)
    {
      ++scan_statistics.inaccessible_entry_count;
      structured_logger_.log_warning(
        "The crawler could not advance to the next entry cleanly.",
        {
  ...
  file_system_catalog_repository_.finish_scan_run(
    scan_statistics.scan_run_identifier,
    scan_statistics.accessible_entry_count,
    scan_statistics.inaccessible_entry_count);
...
{
  const std::time_t timestamp_value = static_cast<std::time_t>(seconds_since_epoch);
...
std::string FileSystemCrawler::create_symbolic_link_target_path_text(const std::filesystem::path &entry_path)
{
  ...
  return std::string(target_path_buffer.data(), static_cast<std::size_t>(bytes_written));
}

FileSystemEntryRecord FileSystemCrawler::create_file_system_entry_record(
  const std::int64_t scan_run_identifier,
  const std::filesystem::path &entry_path,
  const bool should_follow_symbolic_links) const
{
  ...
  {
    throw std::runtime_error("Unable to read file status: " + std::string(std::strerror(errno)));
  }
  ...
  file_system_entry_record.entry_name = entry_path.filename().empty() ? entry_path.string() : entry_path.filename().string();
