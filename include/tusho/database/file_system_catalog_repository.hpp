...
public:
  ...
  void clear_catalog() const;
  std::int64_t begin_scan_run(const std::string& crawl_root_directory_path_text) const;
  void record_file_system_entry(const FileSystemEntryRecord& file_system_entry_record);
  void finish_scan_run(std::int64_t scan_run_identifier, std::uint64_t accessible_entry_count,
                       std::uint64_t inaccessible_entry_count) const;
