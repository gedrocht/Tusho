...
{
inline void expect_condition(const bool condition_is_true, const std::string& failure_message_text,
                             const std::string& source_file_path_text, const int source_line_number)
{
  if (!condition_is_true)
  {
    throw std::runtime_error("Expectation failed at " + source_file_path_text + ":" +
                             std::to_string(source_line_number) + " - " + failure_message_text);
  }
}
